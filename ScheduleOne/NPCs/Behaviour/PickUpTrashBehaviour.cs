...
public class PickUpTrashBehaviour : Behaviour
{
	...
	private void GoToTarget()
	{
		...
		{
			SetDestination(TargetTrash.transform.position);
		}
	}
	...
	{
		return Vector3.Distance(base.Npc.transform.position, TargetTrash.transform.position) <= 2f;
	}
	...
	private bool AreActionConditionsMet(bool checkAccess)
	{
		...
		if (checkAccess && !base.Npc.Movement.CanGetTo(TargetTrash.transform.position, 2f))
		{
			return false;
		}
		return true;
	...
	private void RpcLogic___PerformAction_2166136261()
	{
		...
		IEnumerator Action()
		{
			...
			else
			{
				if (InstanceFinder.IsServer)
				{
					base.Npc.Movement.FacePoint(TargetTrash.transform.position);
				}
