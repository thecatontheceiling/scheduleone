...
public class PlayerCamera : PlayerSingleton<PlayerCamera>
{
	...
	protected virtual void Update()
	{
		...
		if (Player.Local.Schizophrenic)
		{
			...
			{
				timeUntilNextSchizoVoice = UnityEngine.Random.Range(5f, 20f);
				SchizoVoices.VolumeMultiplier = UnityEngine.Random.Range(0.5f, 1f);
				SchizoVoices.PitchMultiplier = UnityEngine.Random.Range(0.4f, 1f);
				SchizoVoices.transform.localPosition = UnityEngine.Random.insideUnitSphere * 1f;
				SchizoVoices.Play();
			}
		}
	...
	protected virtual void LateUpdate()
	{
		...
		if (!fovOverriden && ILerpCameraFOV_Coroutine == null)
		{
			...
			if (MethVisuals)
			{
				FovJitter = Mathf.Lerp(FovJitter, UnityEngine.Random.Range(0f, 1f), Time.deltaTime * 10f);
			}
	...
	private void RotateCamera()
	{
		...
		{
			seizureJitter = Vector2.Lerp(b: new Vector2(UnityEngine.Random.Range(-1f, 1f), UnityEngine.Random.Range(-1f, 1f)), a: seizureJitter, t: Time.deltaTime * 10f);
			num += seizureJitter.x;
		...
		if (SmoothLook)
		{
			mouseX = Mathf.Lerp(mouseX, num, SmoothLookSpeed * Time.deltaTime);
			mouseY = Mathf.Lerp(mouseY, num2, SmoothLookSpeed * Time.deltaTime);
		}
	...
	public void StopTransformOverride(float lerpTime, bool reenableCameraLook = true, bool returnToOriginalRotation = true)
	{
		...
		{
			ILerpCamera_Coroutine = StartCoroutine(ILerpCamera(cameralocalPos_PriorOverride, quaternion, lerpTime, worldSpace: false, returnToRestingPosition: true, reenableCameraLook));
		}
	}
