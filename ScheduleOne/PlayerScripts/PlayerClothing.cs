...
public class PlayerClothing : NetworkBehaviour, IItemSlotOwner
{
	...
	{
		AvatarSettings avatarSettings = UnityEngine.Object.Instantiate(appearanceSettings);
		for (int i = 0; i < avatarSettings.BodyLayerSettings.Count; i++)
		{
			if (!(avatarSettings.BodyLayerSettings[i].layerPath == string.Empty))
			{
				...
				if (!(avatarLayer == null) && avatarLayer.Order > 19 && !TryGetInventoryClothing(avatarLayer.AssetPath, avatarSettings.BodyLayerSettings[i].layerTint, out var _))
				{
		...
		for (int j = 0; j < avatarSettings.AccessorySettings.Count; j++)
		{
			if (!(avatarSettings.AccessorySettings[j].path == string.Empty))
			{
				...
				if (!(accessory == null) && !TryGetInventoryClothing(accessory.AssetPath, avatarSettings.AccessorySettings[j].color, out var _))
				{
