...
public abstract class SphereUtility
{
	...
	{
		return new Vector2(Mathf.Lerp(0f, MathF.PI * 2f, uv.x), Mathf.Lerp(-MathF.PI / 2f, MathF.PI / 2f, uv.y));
	}

	public static Vector2 ConvertSphericalCoordateToUV(Vector2 sphereCoord)
	{
		return new Vector2(sphereCoord.x / (MathF.PI * 2f), (sphereCoord.y + MathF.PI / 2f) / MathF.PI);
	}
}
