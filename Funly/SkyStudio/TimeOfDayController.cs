...
public class TimeOfDayController : MonoBehaviour
{
	...
	public SkyProfile skyProfile
	{
		...
		set
		{
			...
			{
				m_SkyProfile = UnityEngine.Object.Instantiate(value);
			}
			else
			{
				m_SkyProfile = value;
	...
	public bool StartSkyProfileTransition(SkyProfile toProfile, float duration = 1f)
	{
		...
		BlendSkyProfiles blendSkyProfiles = UnityEngine.Object.Instantiate(skyProfileTransitionPrefab);
	...
	{
		BlendSkyProfiles component = GetComponent<BlendSkyProfiles>();
		if (!(component == null))
		{
	...
	{
		if (!GetComponent<BlendSkyProfiles>())
		{
			return false;
		}
		return true;
	...
	public void UpdateSkyForCurrentTime()
	{
		...
		if (skyProfile.IsFeatureEnabled("AmbientLightGradient"))
		{
			...
			RenderSettings.ambientSkyColor = skyProfile.GetColorPropertyValue("AmbientLightSkyColorKey", timeOfDay);
			RenderSettings.ambientEquatorColor = skyProfile.GetColorPropertyValue("AmbientLightEquatorColorKey", timeOfDay);
			RenderSettings.ambientGroundColor = skyProfile.GetColorPropertyValue("AmbientLightGroundColorKey", timeOfDay);
		}
