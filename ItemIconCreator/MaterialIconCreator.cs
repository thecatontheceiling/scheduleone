...
public class MaterialIconCreator : IconCreator
{
	...
	private IEnumerator BuildIconsRotine()
	{
		...
		if (dynamicFov)
		{
			UpdateFOV(targetRenderer.gameObject);
		}
		if (lookAtObjectCenter)
		{
			LookAtTargetCenter(targetRenderer.gameObject);
		}
		...
		for (int i = 0; i < materials.Length; i++)
		{
			...
			{
				IconCreatorCanvas.instance.SetInfo(materials.Length, i, materials[i].name, isRecording: true, nextIconKey);
			}
			if (whiteCam != null)
			{
